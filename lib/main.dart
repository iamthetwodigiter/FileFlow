...
Future<void> _requestPermissions(dynamic deviceInfo) async {
  try {
    ...
    if (androidVersion >= 13) {
      debugPrint('🔒 Android $androidVersion detected - Using MANAGE_EXTERNAL_STORAGE');
      
      final storageStatus = await manageExternalStorage.status;
      debugPrint('🔒 MANAGE_EXTERNAL_STORAGE status: $storageStatus');
      
      if (storageStatus.isDenied) {
        debugPrint('🔒 Permission denied - Requesting MANAGE_EXTERNAL_STORAGE...');
      ...
      if (storageStatus.isDenied) {
        debugPrint('🔒 Permission denied - Requesting WRITE_EXTERNAL_STORAGE...');
